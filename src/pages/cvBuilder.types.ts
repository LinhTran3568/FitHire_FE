...
export function computeCategories(d: CvDraft): CategoryScore[] {
  let con = 0;
  const conT: string[] = [];
  if (d.contact.fullName.trim()) con += 20;
  else conT.push('Thêm họ và tên.');
  if (d.contact.email.trim()) con += 15;
  else conT.push('Thêm email liên lạc.');
  if (d.contact.phone.trim()) con += 10;
  else conT.push('Thêm số điện thoại.');
  if (d.education.school.trim()) con += 25;
  else conT.push('Bổ sung thông tin học vấn.');
  if (d.education.degree.trim()) con += 15;
  if (d.targetRole.trim()) con += 15;
  else conT.push('Nhập vị trí mục tiêu.');
  ...
  const kwT: string[] = [];
  if (d.hardSkills.length >= 5) kw = Math.min(kw + 20, 100);
  else kwT.push('Thêm ít nhất 5 Hard Skills.');
  ...
  let fmt = 0;
  const fmtT: string[] = [];
  if (d.experiences.length > 0) fmt += 35;
  else fmtT.push('Thêm ít nhất 1 mục Kinh nghiệm.');
  if (d.hardSkills.length > 0) fmt += 25;
  else fmtT.push('Bổ sung phần Kỹ năng.');
  if (d.languages.length > 0) fmt += 20;
  else fmtT.push('Thêm ít nhất 1 Ngoại ngữ.');
  ...
  else if (totalDesc > 80) {
    wrt += 30;
    wrtT.push('Viết mô tả chi tiết hơn.');
  } else wrtT.push('Mô tả còn quá ngắn, hãy dùng ✨ AI Optimize.');
  const hasVerbs = d.experiences.some(e =>
    /(phát triển|xây dựng|tối ưu|tăng|giảm|quản lý|triển khai)/i.test(e.description),
  );
  if (hasVerbs) wrt += 30;
  else wrtT.push('Dùng động từ hành động (phát triển, xây dựng, tối ưu...).');
  if (d.achievements.length > 0) wrt += 20;
  else wrtT.push('Thêm Thành tựu.');

  let met = 0;
  const metT: string[] = [];
  const hasNums = d.experiences.some(e =>
    /\d+%|\d+x|giảm|tăng|\d+ (người|user|project)/i.test(e.description),
  );
  if (hasNums) met += 60;
  else metT.push('Thêm con số cụ thể: %, số người dùng...');
  const hAch = d.achievements.filter(a => a.highlight);
  if (hAch.length > 0) met += 40;
  else if (d.achievements.length > 0) {
    met += 20;
    metT.push('Đánh dấu ⭐ thành tựu nổi bật.');
...
export const C_BAR: Record<string, string> = {
...
export const C_BG: Record<string, string> = {
...
export const C_TXT: Record<string, string> = {
